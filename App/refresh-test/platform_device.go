...
func openDisplay() (display, error) {
	...
	fail := func(err error) (display, error) {
		...
		if fastChanged {
			err = errors.Join(err, control("fast_refresh_only", d.previousFast))
		}
		unix.Close(lease)
		return nil, err
	}
	...
	// Every non-full draw uses fast refresh, including the first draw.
	// Attempt restoration even if a sysfs write reports an error.
	fastChanged = true
	if err = control("fast_refresh_only", "1"); err != nil {
		return fail(err)
	}
	go d.readInput()
...
func (d *deviceDisplay) draw(f frame, full bool) (err error) {
	// Deliberately never deduplicate frames: each call measures a real write
	// attempt, and EAGAIN/EBUSY must reach the caller for per-call logging.
	if full {
	...
	for {
		...
		for i, p := range polls {
			...
			for off := 0; off < n; off += 16 {
				...
				// Press events only; releases/repeats and power/sleep stay out.
