...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $projectRoot 'build\catalog-payload\pic\pic'
}
...
if (-not (Test-Path -LiteralPath $fontSource -PathType Leaf)) { throw "Required font is missing: $fontSource" }
[IO.Directory]::CreateDirectory($payloadRoot) | Out-Null
[IO.Directory]::CreateDirectory((Join-Path $payloadRoot 'assets')) | Out-Null
...
Copy-Item -LiteralPath $fontSource -Destination (Join-Path $payloadRoot 'assets\MiSans-Normal.ttf') -Force
$licenseDirectory = Join-Path $payloadRoot 'assets\licenses'
[IO.Directory]::CreateDirectory($licenseDirectory) | Out-Null
Copy-Item -LiteralPath (Join-Path $appRoot 'internal\lowmem\LICENSE') -Destination (Join-Path $licenseDirectory 'lowmem-BSD.txt') -Force
...
if ($LASTEXITCODE -ne 0) { throw 'The pic binary failed MIPS ABI validation.' }
