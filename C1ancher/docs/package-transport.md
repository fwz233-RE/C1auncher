...
Cooldown state is process-wide and deliberately conservative across repository
URLs, including the fixed-IP alias. It assumes the package manager's serialized
transport calls. It is not a cross-process quota coordinator, is not persisted
across program restarts, and does not synchronize independent updater processes.
The server remains authoritative for concurrency and bandwidth enforcement.
